...
parser.add_argument('--blood', nargs=1, type= str, default=sys.stdin, help = 'Path to blood GO.')
...
blood = pd.read_csv(args.blood[0],sep='\t', header=None)
...
all_categories = np.concatenate([np.array(blood[1].unique()),np.array(frontal_cortex[1].unique()),np.array(cerebellum[1].unique()),np.array(horvath[1].unique()),np.array(hannum[1].unique())])
