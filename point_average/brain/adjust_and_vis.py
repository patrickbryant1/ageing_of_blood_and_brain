...
def calc_derivatives(sel, ages, running_averages, marker_values, point_indices,n_clusters):
    ...
    for cl in range(k):
        ...
        for i in cluster_indices:
            ax.plot(np.arange(0,103),sel_ra[i],color=colors[cl],linewidth=1,alpha=min(1,(20/len(cluster_indices))))

        ax.plot(np.arange(0,103), np.median(np.array(sel_ra)[cluster_indices],axis=0),color='k', linewidth=3)
        ax.scatter(ages,np.median(np.array(sel_marker_values)[cluster_indices],axis=0),color='k',s=1)
        #Plot gradients
        ax2.plot(np.arange(0,103), savgol_filter(np.median(np.array(gradients)[cluster_indices],axis=0),window_length=21,polyorder=2),color=colors[cl], linewidth=2, label = cl+1, alpha =0.8)
        #Format plot
